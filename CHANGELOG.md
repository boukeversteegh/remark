...
Restart also no longer trusts that starting a process is the same as replacing itself. It waits for the replacement to survive its own startup, and if it does not, the old window STAYS OPEN and shows what the new one said. A window that closes and never returns is the worst outcome available here — it takes away the way everything else gets tested.
