...
### Restart comes back
A window looked for its port by trying 7333 and the nineteen above it. Windows reserves whole BLOCKS of TCP ports — Hyper-V, WSL and container networking take ranges that are re-randomised at every boot, and a reserved port refuses you with an access-permission error rather than "in use" — so once a block landed on that span, every one of the twenty attempts failed and the process exited before opening anything. Pressing Restart closed the working window and nothing came back. The walk upwards is now only a preference: when it comes to nothing, the OS is asked to name a port it knows is free, which it will not do from inside a reserved range. The gateway already did this; the window does now too, and says which range it had to leave.

Restart also no longer trusts that starting a process is the same as replacing itself. The new window now reports its address as soon as it is serving, and the old one connects to that address before standing down — if the replacement dies, or is still silent when the wait runs out, the old window STAYS OPEN and shows what the new one said. Elapsed time is not evidence: a child still starting when a timer expires would be called a success and then fail with nothing left on screen. A window that closes and never returns is the worst outcome available here — it takes away the way everything else gets tested — so an honest refusal, even one that leaves two windows open, wins over a hopeful handover.

### Fold the opening post without folding the thread
A titled thread now has two handles, and they answer different questions. The one in the title bar puts the whole thread away, as before. The one beside the opening post — its caret, its header row, its gutter strip — puts away only that comment's text, leaving the title, the replies and the thread's shape on screen. A long opening post was the thing that kept a board from being compact even after everything read was folded, and "fold read comments" now folds a read opening post while a thread held open by an unread reply keeps that reply exactly where it was. The two states are independent: fold the thread, open it again, and the opening post is the way you left it. An untitled thread keeps its single handle — there is no header row to hang a second one on. Folding never hides what you are being shown: a search match in the post, an open composer in it, or a comment a jump just landed on.

### The Authors panel follows the date filter
Set a window and the panel answers the same question the board does: who is part of THIS window. An offline author with nothing inside it drops out of the list — not because they left, but because there is nothing of theirs on screen to attribute. Anything of theirs counts, including a bare emoji reaction, and a window in the past keeps the people who were in it. Online authors always stay: whether an agent is listening is a question about now, not about the filter, and answering it is the panel's main job. The list never shrinks silently — a line at the bottom says how many it dropped, and clicking it clears the filter.

### A Claude Code plugin that refuses a monitor started from a shell
remark monitor is a stream, and started from an ordinary shell call it is either killed when that call times out or has its output held until it dies — either way the comments never arrive, and nothing about the failure is visible from the outside. A plugin under claude-plugin/ carries a PreToolUse guard that catches it before it happens: remark hook claude reads the tool call and denies a monitor launch with an explanation pointing at the streaming tool. It reads the command the way a shell would rather than matching text in it, so what counts is the program in command position and the verb after it: quoted Windows paths, chained and backgrounded commands and `bash -c` strings are recognised, while anything that merely mentions the words is left alone — remark help monitor, an echo, a grep, a -text search, a program whose name happens to end in remark. What it cannot work out it allows: an unresolved `$VAR`, an unterminated quote, a wrapper it does not know. Include REMARK_HOOK_OK in a command to run one from a shell deliberately. The plugin starts nothing on installation: which document an agent follows, under which name, stays a decision someone makes.
